...
export const fetchCodeChefContests = async () => {
  try {
    ...
    const fetchBatch = async (batchOffset: number) => {
      try {
        const response = await axios.get(
          `https://www.codechef.com/api/list/contests/past?sort_by=START&sorting_order=desc&offset=${batchOffset}&mode=all`
        );
    ...
    for (let i = 0; i < batchesToFetch.length; i += concurrentRequests) {
      const currentBatchOffsets = batchesToFetch.slice(
        i,
        i + concurrentRequests
      );

      const batchResults = await Promise.all(
        currentBatchOffsets.map((offset) => fetchBatch(offset))
      );
    ...
    function mapContests(contests: any[]) {
